...
export default class Bootstrap extends Phaser.Scene {
  ...
  preload() {
    ...
    // Office Decorations & Environment
  ...
  /**
   * Called by RoomSelectionDialog or LoginDialog once the network is ready.
   */
  launchGame() {
    if (!this.preloadComplete) {
      console.warn('Assets not yet loaded!')
      return
    }

    this.network.webRTC?.checkPreviousPermission()
    
    // Launch the main game scene and pass the network instance
    this.scene.launch('game', {
      network: this.network,
    })

    // Update Redux state so React UI knows we are in the Meatverse
    store.dispatch(setRoomJoined(true))
  }

  changeBackgroundMode(backgroundMode: BackgroundMode) {
    if (this.scene.isActive('background')) {
